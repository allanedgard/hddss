 ...
 * @author allan
 */
public class ConjuntoInteiro  implements Comparable<ConjuntoInteiro> {
    ...
    @Override public boolean equals( Object aThat ) {
     ...
     if ( !(aThat instanceof ConjuntoInteiro) ) return false;

     ConjuntoInteiro that = (ConjuntoInteiro)aThat;
     return
   ...
   @Override public int hashCode() {
    ...
    public int compareTo( ConjuntoInteiro aThat ) {
        final int BEFORE = -1;
        final int EQUAL = 0;
        final int AFTER = 1;
    ...
    ConjuntoInteiro interseccao(ConjuntoInteiro x) {
   ...
   void adiciona(ConjuntoInteiro x) {
        for (int i=0; i<x.c.size(); i++) {
            int v = ((Integer) x.c.get(i)).intValue();
            if (!this.existe(v))
               this.adiciona(v);
