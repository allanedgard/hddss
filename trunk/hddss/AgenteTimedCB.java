public class AgenteTimedCB extends SimulatedAgent {
   ...
   //     char [] [] BM_Matrix;
        ...
        ConjuntoInteiro live;
        ConjuntoInteiro uncertain;
        ConjuntoInteiro suspected;
        ConjuntoInteiro visao;
 ...
 //           BM_Matrix = new char[controle.n][controle.tempofinal*2];
             ...
             */
            down = new ConjuntoInteiro();
            live = new ConjuntoInteiro();
            uncertain = new ConjuntoInteiro();
            suspected = new ConjuntoInteiro();
            visao = new ConjuntoInteiro();
            ...
            Consensus = new Consenso[infra.context.tempofinal*2];
...
//        };
       ...
       void registraBloco(int numbloco, int sender, int ti) {
        ...
        public void execute() {
            ...
            if ( (r.uniform() <= prob)  ) {
                ...
                enviaMensagemGrupo(clock, TIMEDCB_APP, new ConteudoTimedCB("payload", LCB[id], acks), LogicalClock, true );
            ...
            if ( (RECV > SENT) || (bloquearEntrega) ) {
                if (clock - LastTimeSent >= ts) {
                        ...
                        enviaMensagemGrupo(clock, TIMEDCB_TS, new ConteudoTimedCB("time-silent", LCB[id], acks), LogicalClock );
                        registraBloco(LogicalClock, id, clock);
                }
            }
        ...
        public void enviaMensagemGrupo(int clock, int tipo, Object valor, int LC) {
            for (int j=0; j<infra.nprocess;j++){
                ...
                if(valor instanceof ConteudoTimedCB){
                    ((ConteudoTimedCB)valor).vack[j].rsendTime = clock;
                }
     ...
     */ 
      ...
      */
     public ConjuntoInteiro retornaExpirados(int bl) {
     ...
     void verificaCompletudeCausalBlock() {
        ...
        for (int i=0; i<bufferDeMensagens.size();i++) {
                m = (Mensagem) bufferDeMensagens.get(i);
        ...
        for (int i=1; i<= bufferDeMensagensInstaveis.size();i++) {
            m = (Mensagem) bufferDeMensagensInstaveis.get(i-1);
        ...
        for (int i=1; i<=bufferDeMensagens.size();i++) {
            m = (Mensagem) bufferDeMensagens.get(i-1);
        ...
        public void receive(Mensagem msg) {
            ...
            switch (msg.tipo) {
                ...
                case TIMEDCB_TS:
                    ...
                    if ( ((ConteudoTimedCB) msg.conteudo).LCB > LCB[msg.remetente]) { 
                            LCB[msg.remetente] = ((ConteudoTimedCB) msg.conteudo).LCB;
                        }
                ...
                case CHANGE_VIEW_REQUEST:
                    ...
                    uC = new UnstableContent(b, bufferDeMensagensInstaveis, msg.remetente, down);
                    enviaMensagemGrupo(clock, UNSTABLE, uC, LogicalClock);
                    break;
                case UNSTABLE:
                    if (!consenso) {
                        uC = (UnstableContent) msg.conteudo;
                    ...
                    break;
                case CONSENSUS_P1:
                    c = (Consenso) msg.conteudo;
                    // Mensagem do mesmo round
                    if (Consensus[c.numero]==null)
                        Consensus[c.numero] = iniciaConsensus( (UnstableContent) c.estimado );
                ...
                case CONSENSUS_P2:
                    ...
                    if (!Consensus[c.numero].atingiuQuorum) {
                        ...
                        if ( ((UnstableContent) c.estimado).tamanho() != 0)
                                Consensus[c.numero].rec = c.estimado;
                        if (obtidoQuorumConsenso(c.numero)) {
                            if ( ((UnstableContent) c.estimado).tamanho() == 0)
                                Consensus[c.numero].noneREC = true;               
                            if ( ( (UnstableContent) Consensus[c.numero].rec).conteudo.size() == 0)
                            {
                ...
                case DECIDED:
                    ...
                    if (Consensus[c.numero].ativo) {
                        ...
                        uC  =  (UnstableContent) Consensus[c.numero].estimado;
                        java.util.ArrayList x = (java.util.ArrayList) uC.conteudo;
                        for (int i=0;i<x.size();i++) {
                            Mensagem m = (Mensagem) x.get(i);
         ...
         */
        Consenso iniciaConsensus(UnstableContent uC) {
            Consenso c = new Consenso(uC.bloco,0,uC);
            return c;
        }
         ...
         */
        void adicionaVisao(UnstableContent uC) {
         ...
         */
        void adicionaREC(int numero, UnstableContent uC) {
            for (int i = 0; i < uC.conteudo.size(); i++ )
